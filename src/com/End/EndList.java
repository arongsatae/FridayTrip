...
public class EndList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		ArrayList<EndListDTO> list = dao.list(start, end); // 리스트 목록		
		...
		for (int i=0; i<list.size(); i++) {
			EndListDTO dto = list.get(i);//게시물1개
			...
			if ((span / 1000 / 60 / 60) < 18) {
				// 최신글... 뉴 이미지
				dto.setNewImg("<img src='Trip/images/010.PNG' align='center'/>");
			}	
		...
		if (temp2 == 1) { 
			...
			// 반대
			pageBar += String.format("<a href='/FridayTrip/Trip/EndList.do?"
		...
		while (!(loop > blockSize || temp2 > totalPage)) {
			if (temp2 == nowPage) { // 현재 페이지
				...
				pageBar += String.format("<a href='/FridayTrip/Trip/EndList.do?page=%d'>%d</a>", temp2, temp2);
			}
			loop++;
			temp2++;
		}
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/Trip/EndList.jsp");
