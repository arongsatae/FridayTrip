...
public class FreeList extends HttpServlet {
   ...
   protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
      ...
      ArrayList<FreeListDTO> list = dao.list(start, end); // 리스트 목록      
      ...
      for (int i=0; i<list.size(); i++) {
         FreeListDTO dto = list.get(i);//게시물1개
         ...
         if(dto.getDepth() == null){
            depth2 = "0";
            depth = Integer.parseInt(depth2);
         }
         // 새글 ? 답변글 ?
         System.out.println("d : " + depth2);
      ...
      if (temp2 == 1) { 
         // 현재 페이지가 첫번째 블럭이냐 ?
         pageBar += String.format("<img src='/FridayTrip/Trip/images/left.JPG' alt='이전'"
         + " style='cursor: pointer; width: 10px; height:10px; padding-right: 10px;'/>", blockSize);
      } else {
         // 반대
         pageBar += String.format("<a href='/FridayTrip/Trip/FreeList.do?"
         + "page=%d'><img src='/FridayTrip/Trip/images/left.JPG' alt='이전'"
      ...
      while (!(loop > blockSize || temp2 > totalPage)) {
         if (temp2 == nowPage) { // 현재 페이지
            ...
            pageBar += String.format("<a href='/FridayTrip/Trip/FreeList.do?page=%d'>%d</a>", temp2, temp2);
         }
         loop++;
         temp2++;
      }
      ...
      if (temp2 > totalPage) {
         // 마지막 블럭에 속한 페이지(끝페이지)
         pageBar += String.format("<img src='/FridayTrip/Trip/images/right.JPG' alt='다음'"
         + " style='cursor: pointer; width: 10px; height:10px; padding-left: 10px;'/>", blockSize);
      } else {
         // 마지막 블럭에 속하지 않은 페이지
         pageBar += String.format("<a href='/FridayTrip/Trip/FreeList.do?"
               + "page=%d'><img src='/FridayTrip/Trip/images/right.JPG' alt='다음'"
      ...
      RequestDispatcher dispatcher = req.getRequestDispatcher("/Trip/FreeList.jsp");
